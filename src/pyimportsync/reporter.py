...
class DependencyReporter:
    ...
    def print_verbose_info(self, project_root: Path, requirements_file: str, 
                          has_pathspec: bool, has_rich: bool, cache_enabled: bool,
                          ignore_dirs: Set[str], pattern_count: int):
    ...
    def generate_report(self, analysis_results: Dict[str, List[str]]) -> int:
        ...
        if self.verbose:
            print(f"Found {len(set(missing + [m.split(' → ')[0] for m in matched]))} unique imports in code")
        ...
        if HAS_RICH and self.console:
            return self._generate_rich_report(missing, matched, pipreqs, imports_count, requirements_count)
        else:
            return self._generate_plain_report(missing, matched, pipreqs, imports_count, requirements_count)
    
    def _generate_rich_report(self, missing: List[str], matched: List[str], 
                             pipreqs: List[str], imports_count: int, requirements_count: int) -> int:
        """Generate rich formatted report."""
        self.console.print("\n" + "=" * 60)
        self.console.print("🔍 [bold blue]PyImportSync DEPENDENCY ANALYSIS REPORT[/bold blue]")
        ...
        self.console.print(f"  • Requirements packages: [cyan]{requirements_count}[/cyan]")
    ...
    def _generate_plain_report(self, missing: List[str], matched: List[str], 
                              pipreqs: List[str], imports_count: int, requirements_count: int) -> int:
