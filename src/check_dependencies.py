...
def main():
    parser = argparse.ArgumentParser(description="PyImportSync - Python Import Synchronization Tool")
    parser.add_argument("--project-root", default=None, help="Project root directory")
    parser.add_argument("--requirements-file", default="requirements.txt", help="Requirements file path")
    ...
    parser.add_argument("--respect-gitignore", action="store_true", default=True, help="Respect .gitignore patterns")
    parser.add_argument("--no-gitignore", action="store_true", help="Ignore .gitignore patterns")
    parser.add_argument("--ignore-dirs", default="", help="Comma-separated list of directories to ignore")
    parser.add_argument("--output", default="", help="Output file to save missing dependencies")
    ...
    if args.ignore_dirs:
        additional_ignore_dirs = [d.strip() for d in args.ignore_dirs.split(',') if d.strip()]
        # Load default config to get existing ignore dirs
        from pyimportsync.utils import load_config
        default_config = load_config(None, None)
        merged_ignore_dirs = list(set(default_config['ignore_dirs'] + additional_ignore_dirs))
    ...
    if args.output and result != 0:
        try:
            ...
            analysis_results = checker.analyze_dependencies(use_pipreqs=not args.no_pipreqs)
